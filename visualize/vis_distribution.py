...
with open('result_scripts/loss_vs_privacy_occupancy_departure_public_deep_departure_mc.pickle', 'rb') as f:
    s, losses,ss, losses_best, losses_generic = pickle.load(f)
...
for i in anonymity_vec:
    sanitized_profile_best_list[i], sanitized_profile_baseline_list[i], sanitized_profile_list[i], sanitized_profile_deep_list[i] = s[(i,0)]
...
for i in anonymity_vec:
    ...
    # dep_mean_best = np.mean(depart_best.values)
    #
    ...
    # err_best = abs(dep_mean_best - dep_mean_gt) / dep_mean_gt
    #
    # print('-----------------')
    # print(i)
...
mc_num = 5
tv_gn = np.empty((n_a,mc_num))
tv_best = np.empty((n_a,mc_num))
tv_sd = np.empty((n_a,mc_num))
tv_sn = np.empty((n_a,mc_num))
for mc_i in range(mc_num):
    ...
    sanitized_profile_deep_list = {}
    for i in anonymity_vec:
        sanitized_profile_best_list[i], sanitized_profile_baseline_list[i], sanitized_profile_list[i], sanitized_profile_deep_list[i] = s[(i,mc_i)]
    ...
    for ai in range(n_a):
        ...
        tv_gn[ai,mc_i] = np.linalg.norm(dep_hist_gt-dep_hist_gn,ord=1)
        tv_sn[ai,mc_i] = np.linalg.norm(dep_hist_gt - dep_hist_sn, ord=1)
        tv_sd[ai,mc_i] = np.linalg.norm(dep_hist_gt - dep_hist_sd, ord=1)
        tv_best[ai,mc_i] = np.linalg.norm(dep_hist_gt - dep_hist_best, ord=1)
...
for mc_i in range(mc_num):
    plt.figure()
    plt.plot(anonymity_vec, tv_gn[:,mc_i], label='generic')
    plt.plot(anonymity_vec, tv_best[:,mc_i], label='best')
    plt.plot(anonymity_vec, tv_sn[:,mc_i], label='linear')
    plt.plot(anonymity_vec, tv_sd[:,mc_i], label='deep')
    plt.legend()
    plt.show()
...
for i in anonymity_vec:
    sanitized_profile_best_list[i], sanitized_profile_baseline_list[i], sanitized_profile_list[i], \
    sanitized_profile_deep_list[i] = s[(i, 0)]
