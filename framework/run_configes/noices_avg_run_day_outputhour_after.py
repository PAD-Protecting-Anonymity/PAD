...
for i in range(0,7):
    ...
    framework = Framework(data,anonymity_level,rep_mode=rep_mode, resample_factor = min_resample_factor,learning_metric=MetricLearningTerms.NONLINEAR, k_fold= k_fold,output_groupper_after=True, all_data=all_data, all_sampling_rates= all_samplingRates)
    ...
    pickle.dump([out, framework.generated_data_description(), loss_metric,anonymity_level,timeTaken], open('./results/noices/after/PAD_results_noices_avg_dayli_mean_NONLINEAR_OutputHour_fold_'+str(i)+'.pickle', "wb"))
