...
for n in [2]:#range(2,8):
    s, l, ss, dist_relation_lm, dist_relation_dm = evaluation_occupancy_statistics(n, mode)
    sanitized[n] = s
    losses[n] = l
...
with open('../result_scripts/test_deep_presanitized', 'wb') as f:
        pickle.dump([sanitized, losses, sample_sizes,dist_lm,dist_dm], f)

with open('./result_scripts/test_deep_presanitized', 'rb') as f:
